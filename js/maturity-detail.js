...
function renderLandCard(land, now, displayLandId) {
  ...
  return `<div class="${cls.join(' ')}" title="土地${displayLandId}: ${maturityEsc(plantName)}\n成熟：${maturityEsc(maturityFmtTime(matureAt))}\n剩余：${left}/${total}">
    ...
    <div class="farm-info">
      ...
      ${hasPlant ? `<div class="farm-meta">熟 ${maturityEsc(shortTime(matureAt))} · 余 ${left}/${total}</div><div class="farm-countdown ${ready ? 'over' : ''}">${maturityEsc(maturityFmtCountdown(matureAt))}</div>${isMutant ? `<div class="farm-countdown">${maturityEsc(land.mutant_summary)}</div>` : ''}` : ''}
    </div>
  </div>`;
}
